...
## Concordance — `correctness/concordance.R <vcf> <gff3> [--fasta ref.fa] [--vep vep.tsv]`
...
## Benchmark — `scripts/bench.R <vcf> <gff3> [--fasta ref.fa]`

End-to-end throughput, duckvep vs fastVEP (hyperfine if installed, else `time`).

## Full-size GIAB — `scripts/fetch-data.R`
...
scripts/bench.R       data/giab/HG002.vcf.gz data/giab/GRCh38.116.gff3.gz --fasta data/giab/GRCh38.fa
correctness/concordance.R data/giab/HG002.vcf.gz data/giab/GRCh38.116.gff3.gz --fasta data/giab/GRCh38.fa
```
