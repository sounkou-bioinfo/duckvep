...
- `build-cache.R` — downloads dumps + loads them into a local DuckDB (stage 1),
...
Counts below are **generated from the built cache** (`cache_stats.csv`), so they
match the data exactly:

```{r echo=FALSE, message=FALSE}
s <- read.csv("correctness/cache-build/data/cache_stats.csv", stringsAsFactors = FALSE)
meta <- paste(readLines("correctness/cache-build/data/cache_build_meta.txt"), collapse = " ")
knitr::kable(s[order(-s$n), ], col.names = c("metric", "count"), row.names = FALSE,
             format.args = list(big.mark = ","), caption = meta)
```
