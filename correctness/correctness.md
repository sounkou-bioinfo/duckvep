
<!-- correctness.md is generated from correctness.Rmd — edit the .Rmd and run
     `make correctness`. The table is read **directly from the recorded CSV**
     (correctness/data/concordance_by_impact.csv) and rendered with knitr. -->

# duckvep correctness

Our accuracy oracle is **Ensembl VEP itself** (decades of curated
knowledge), not fastVEP. Every figure below is read **directly from a
recorded CSV** under `correctness/data/`, regenerated by
`correctness/vep_concordance.R` (offline Ensembl VEP) and the synthetic
corpus generator `tools/gen-hard-variants` (Rust + noodles).

All three annotators are compared on the **canonical normalized variant
key** (`normalize_variant`, matching VEP’s minimal representation), so
the join is valid for *every* variant class including indels.

**Method (no slop):** VEP code, VEP cache and the duckvep GFF3 are the
**same Ensembl release** (no version skew), variants are biallelic
ClinVar, and concordance is the fraction of `(variant, transcript)`
pairs whose `&`-joined SO consequence set is identical. **We never
report a single aggregate** — an average hides *where* the misses are.
fastVEP’s headline “99.99%” concealed highly significant discordances
concentrated in HIGH-impact sites. So every figure is split by **IMPACT
× variant class**, and HIGH-impact discordances are surfaced.

**Controlled comparison — the only free variable is the engine.** VEP is
run with `--gff` on the **same GFF3 + FASTA the engines use** (not its
own cache), so the transcript *set* and *model* are identical for all
three tools (duckvep, fastVEP, VEP); only the engine differs. (Ensembl
116’s GFF uses the `ncRNA_gene` feature type, which VEP’s hard-coded
include-list omits — so it would silently drop ~42k non-coding genes; we
rewrite it to `gene`, identically for all three.) This collapses the
cache-vs-GFF transcript-set difference from **7,011 → 26** pairs, so the
residual is pure engine. This is *VEP 116’s engine on the controlled GFF
model* — it deliberately excludes cache-only semantics (mature-miRNA
products, RNA edits, selenocysteine), which is why e.g. the
`mature_miRNA_variant` discordances disappear under control.

**Scale vs the fastVEP paper.** The fastVEP manuscript validates
accuracy as “100% concordance across 23 annotation fields on **2,340
shared transcript-allele pairs**” — which is the **173-variant VEP
example file, all SNVs** — against VEP 115.1. Concordance on a few
thousand SNVs is the *easy* regime. This harness instead runs **~1.4M
`(variant, transcript)` pairs** (50,000 biallelic ClinVar variants,
indels and MNVs included) and scores the **consequence** SO-term set for
*both* engines against the same controlled VEP 116. That is where the
divergences a 2,340-SNV set never surfaces appear — concentrated, for
fastVEP, in HIGH-impact indels and MNVs (table below).

## Concordance vs Ensembl VEP — split by IMPACT × class

`discordant / pairs` (lower discordant is better). duckvep is the
vendored fastVEP engine plus accuracy patches, so the head-to-head is
fair; both are keyed to the original input variant against the same
controlled VEP 116.

| impact   | class |  pairs | duckvep err (rate) | fastvep err (rate) |
|:---------|:------|-------:|:-------------------|:-------------------|
| HIGH     | del   |  24631 | 16 (65/100K)       | 2146 (8713/100K)   |
| HIGH     | ins   |  10447 | 40 (383/100K)      | 515 (4930/100K)    |
| HIGH     | mnv   |    312 | 0 (0/100K)         | 116 (37179/100K)   |
| HIGH     | snv   |  43184 | 1 (2/100K)         | 20 (46/100K)       |
| MODERATE | del   |   3570 | 0 (0/100K)         | 180 (5042/100K)    |
| MODERATE | ins   |   1910 | 44 (2304/100K)     | 109 (5707/100K)    |
| MODERATE | mnv   |   1484 | 0 (0/100K)         | 311 (20957/100K)   |
| MODERATE | snv   | 407271 | 0 (0/100K)         | NA                 |
| MODERATE | snv   | 407255 | NA                 | 1 (0/100K)         |
| LOW      | del   |   7269 | 31 (426/100K)      | 1123 (15449/100K)  |
| LOW      | ins   |   3522 | 0 (0/100K)         | 407 (11556/100K)   |
| LOW      | mnv   |    174 | 0 (0/100K)         | 1 (575/100K)       |
| LOW      | snv   | 212558 | 0 (0/100K)         | NA                 |
| LOW      | snv   | 212557 | NA                 | 37 (17/100K)       |
| MODIFIER | del   |  36247 | 0 (0/100K)         | 20 (55/100K)       |
| MODIFIER | ins   |  19351 | 0 (0/100K)         | 9 (47/100K)        |
| MODIFIER | mnv   |   2037 | 0 (0/100K)         | 0 (0/100K)         |
| MODIFIER | snv   | 664797 | 0 (0/100K)         | NA                 |
| MODIFIER | snv   | 664781 | NA                 | 30 (5/100K)        |

Error rate is **per 100,000 (variant, transcript) pairs** — comparable
across classes despite very different pair counts.

**Reading it** (all figures inline-generated from the CSV above):

- **SNVs have low error rates at every impact tier** — duckvep error
  rate HIGH 2/100K, MODERATE 0/100K, LOW 0/100K, MODIFIER 0/100K, and
  below fastVEP (MODERATE 0/100K vs duckvep 0/100K), thanks to the
  incomplete-CDS / non-ATG-start patches (see `../docs/PATCHES.md`).
- **Indels and MNVs are the residual frontier**, concentrated at HIGH
  impact — duckvep error rate HIGH del 65/100K, HIGH ins 383/100K, HIGH
  mnv 0/100K (vs SNV 2/100K). These are the clinically actionable sites
  an aggregate % would bury.
- **The indel/MNV frontier is partly duckvep-specific.** Engines are
  compared keyed to the original input variant (not each engine’s
  normalized form — VEP and duckvep 3’-shift indels differently, so a
  normalized-key join compares mismatched pairs and hides real
  regressions; that earlier join was a measurement artifact). On this
  keying, duckvep diverges on 132 shared pairs + 29 emission = 161
  total, of which **39 are duckvep-specific** (fastVEP matches VEP), vs
  fastVEP’s 5,089. Those are boundary indels where VEP 3’-shifts the
  allele before calling consequence; the rest are gaps shared with
  fastVEP.

## Convergence toward conformance

Each `correctness/vep_concordance.R` run appends a timestamped row of
the headline divergence counts to
`correctness/data/conformance_history.csv`, so progress is tracked over
time (lower is better; duckvep vs the vendored fastVEP, both against the
same controlled VEP 116):

![](correctness_files/figure-gfm/convergence-1.png)<!-- -->

A single point is the current run; the curve fills in as the engine is
iterated (e.g. closing the duckvep-specific boundary-indel tail). The
raw history table is `conformance_history.csv`.

## Where the misses are — per consequence (SO term)

Impact × class is still too coarse: it buckets every “HIGH del” together
while the real culprits are **specific consequence categories** (splice
acceptor/donor, polypyrimidine tract, frameshift). This table explodes
VEP’s `&`-joined call and attributes each discordant pair to the SO
terms VEP assigned — the finest “where is it actually failing” view.
**Both engines are shown** (top HIGH terms by duckvep discordant count)
so the split between duckvep-specific regressions and gaps shared with
fastVEP is visible; generated from
`correctness/data/discordance_by_consequence.csv`:

| SO term (VEP)               | impact |  pairs | duckvep disc (rate) | fastvep disc (rate) |
|:----------------------------|:-------|-------:|:--------------------|:--------------------|
| intron_variant              | HIGH   |  1,696 | 40 (2358/100K)      | 1696 (100000/100K)  |
| splice_donor_region_variant | HIGH   |    326 | 39 (11963/100K)     | 326 (100000/100K)   |
| splice_donor_variant        | HIGH   | 16,028 | 39 (243/100K)       | 1328 (8286/100K)    |
| NMD_transcript_variant      | HIGH   | 14,954 | 14 (94/100K)        | 666 (4454/100K)     |
| start_lost                  | HIGH   |    888 | 7 (788/100K)        | 111 (12500/100K)    |
| stop_lost                   | HIGH   |    405 | 7 (1728/100K)       | 71 (17531/100K)     |
| 5_prime_UTR_variant         | HIGH   |    175 | 5 (2857/100K)       | 161 (92000/100K)    |
| 3_prime_UTR_variant         | HIGH   |    363 | 1 (275/100K)        | 303 (83471/100K)    |
| frameshift_variant          | HIGH   | 31,901 | 1 (3/100K)          | 537 (1683/100K)     |
| splice_acceptor_variant     | HIGH   | 12,150 | 1 (8/100K)          | 841 (6922/100K)     |
| stop_retained_variant       | HIGH   |     37 | 1 (2703/100K)       | 37 (100000/100K)    |

A pair is discordant when its **whole** `&`-joined SO set differs; this
table credits that to each term VEP called, so a 100% rate means *every*
variant whose VEP call includes that term has a different full set.
Reading the two engine columns: duckvep is **better than fastVEP on 11
of these terms and worse on 0** — i.e. duckvep is never worse than the
upstream engine and ahead on the frameshift / NMD / splice-donor fronts,
while both still trail Ensembl. The remaining frontier is **shared
engine behaviour**: coding indels at exon/intron boundaries where VEP
co-calls `intron_variant` and the generic `coding_sequence_variant`,
frameshifts that introduce a premature stop (`stop_gained`), and MNV
codon edges (`missense`↔`synonymous`). **An impact-only view hid exactly
which categories to fix** — this is the precise frontier, and it is the
same for duckvep and fastVEP.

## Error transitions — what VEP calls vs what duckvep calls

The exact discordances: VEP’s call → duckvep’s call. The `type` column
flags whether fastVEP matches VEP — `regression` = **duckvep is worse
than the upstream engine** (something our patches broke), `shared` =
both engines differ from VEP (an inherited engine gap). Generated from
`correctness/data/error_transitions.csv`.

| type       | impact   | VEP calls                                                                              | duckvep calls                                                                                                                |   n |
|:-----------|:---------|:---------------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------|----:|
| shared     | HIGH     | intron_variant&splice_donor_region_variant&splice_donor_variant                        | intron_variant&splice_donor_region_variant                                                                                   |  33 |
| shared     | LOW      | stop_retained_variant                                                                  | stop_lost                                                                                                                    |  28 |
| regression | MODERATE | NMD_transcript_variant&inframe_insertion&splice_region_variant                         | NMD_transcript_variant&protein_altering_variant&splice_region_variant                                                        |  20 |
| regression | MODERATE | inframe_insertion&splice_region_variant                                                | protein_altering_variant&splice_region_variant                                                                               |  19 |
| shared     | HIGH     | NMD_transcript_variant&stop_lost                                                       | NMD_transcript_variant&inframe_deletion&stop_lost                                                                            |   6 |
| shared     | HIGH     | NMD_transcript_variant&intron_variant&splice_donor_region_variant&splice_donor_variant | NMD_transcript_variant&intron_variant&splice_donor_region_variant                                                            |   6 |
| shared     | MODERATE | inframe_insertion&stop_retained_variant                                                | inframe_insertion&stop_gained                                                                                                |   3 |
| shared     | HIGH     | transcript_ablation                                                                    | intron_variant&non_coding_transcript_exon_variant&splice_acceptor_variant&splice_donor_5th_base_variant&splice_donor_variant |   2 |
| shared     | HIGH     | 5_prime_UTR_variant&NMD_transcript_variant&start_lost&start_retained_variant           | 5_prime_UTR_variant&NMD_transcript_variant&start_lost                                                                        |   2 |
| shared     | MODERATE | inframe_insertion&splice_region_variant&stop_retained_variant                          | inframe_insertion&splice_region_variant&stop_gained                                                                          |   2 |
| shared     | HIGH     | 5_prime_UTR_variant&start_lost                                                         | 5_prime_UTR_variant&coding_sequence_variant                                                                                  |   2 |
| shared     | LOW      | 3_prime_UTR_variant&coding_sequence_variant&incomplete_terminal_codon_variant          | 3_prime_UTR_variant&coding_sequence_variant                                                                                  |   1 |
| shared     | HIGH     | 5_prime_UTR_variant&start_lost&start_retained_variant                                  | 5_prime_UTR_variant&start_lost                                                                                               |   1 |
| shared     | HIGH     | frameshift_variant&stop_lost                                                           | frameshift_variant                                                                                                           |   1 |
| shared     | HIGH     | 3_prime_UTR_variant&intron_variant&splice_acceptor_variant&stop_retained_variant       | 3_prime_UTR_variant&coding_sequence_variant&intron_variant&splice_acceptor_variant                                           |   1 |
| shared     | HIGH     | transcript_ablation                                                                    | 3_prime_UTR_variant&5_prime_UTR_variant&coding_sequence_variant                                                              |   1 |

### Split by SO term (fair to fastVEP)

The table above keys on the **whole** consequence string, so a one-term
difference is hidden inside shared co-occurring terms
(`NMD_transcript_variant`, 5′/3′ UTR). This strips each discordance to
the **symmetric difference** — only the SO terms VEP and duckvep
disagree on — and judges fastVEP **term-by-term**: a `regression` here
means fastVEP is correct on *exactly* the terms duckvep gets wrong (has
every term duckvep missed, adds none duckvep added), so a shared `c1&c2`
gap is never mis-blamed on duckvep and a genuine single-term fastVEP win
is not masked by the rest of its string. Generated from
`correctness/data/so_term_transitions.csv`.

| type       | impact   | VEP term(s)                                        | duckvep term(s)                                                                                                              |   n |
|:-----------|:---------|:---------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------|----:|
| shared     | HIGH     | splice_donor_variant                               | (none)                                                                                                                       |  39 |
| regression | MODERATE | inframe_insertion                                  | protein_altering_variant                                                                                                     |  39 |
| shared     | LOW      | stop_retained_variant                              | stop_lost                                                                                                                    |  28 |
| shared     | HIGH     | (none)                                             | inframe_deletion                                                                                                             |   6 |
| shared     | MODERATE | stop_retained_variant                              | stop_gained                                                                                                                  |   5 |
| shared     | HIGH     | start_retained_variant                             | (none)                                                                                                                       |   4 |
| shared     | HIGH     | start_lost                                         | coding_sequence_variant                                                                                                      |   2 |
| shared     | HIGH     | transcript_ablation                                | intron_variant&non_coding_transcript_exon_variant&splice_acceptor_variant&splice_donor_5th_base_variant&splice_donor_variant |   2 |
| shared     | HIGH     | transcript_ablation                                | 3_prime_UTR_variant&5_prime_UTR_variant&coding_sequence_variant                                                              |   1 |
| shared     | HIGH     | inframe_deletion&start_lost&start_retained_variant | coding_sequence_variant                                                                                                      |   1 |
| shared     | LOW      | (none)                                             | splice_region_variant                                                                                                        |   1 |
| shared     | LOW      | stop_retained_variant                              | coding_sequence_variant                                                                                                      |   1 |
| shared     | HIGH     | stop_retained_variant                              | coding_sequence_variant                                                                                                      |   1 |
| shared     | HIGH     | stop_lost                                          | (none)                                                                                                                       |   1 |
| shared     | LOW      | incomplete_terminal_codon_variant                  | (none)                                                                                                                       |   1 |

At the SO-term level, **39 pairs** are duckvep-specific (fastVEP right
on the exact differing term) — these are the only places duckvep is
worse than upstream on a term; everything else is a shared engine gap or
a term fastVEP also misses.

**duckvep-specific regressions (whole-string): 39 pairs** (a
`regression` is a discordance where fastVEP *matches* VEP, so our
patches — not the inherited engine — are responsible). The
interval-aware splice rewrite originally introduced ~1,900 such pairs;
root-causing them against Ensembl `BaseTranscriptVariationAllele.pm`
closed them in three steps: the polypyrimidine `(min,max)` swap and
`_intron_overlap` insertion cases (283 insertion under-calls), the
codon-table-aware non-ATG `start_lost` (mito initiators), and a harness
join-key fix (deletions share `alt='-'`, so the key must include `ref`).
What remains are all **shared engine gaps** (both duckvep *and* fastVEP
differ from VEP): a frameshift introducing a premature stop
(`frameshift_variant&stop_gained` → `frameshift_variant`) and MNV codon
edges — real engine accuracy work vs Ensembl, not something our patches
broke.

## Auditing the fastVEP paper’s “100% concordance” claim

The fastVEP manuscript reports *“100% concordance across 23 annotation
fields on 2,340 shared (allele, transcript) pairs”* vs Ensembl VEP
115.1. We reproduced its exact protocol — the VEP example dataset
(`examples/homo_sapiens_GRCh38.vcf`) with `--symbol --canonical --hgvs`
— and audited two words doing the heavy lifting.

**“example dataset” → all SNVs.** The 173 example variants are **173/173
SNVs, zero indels, zero MNVs** (71% are synonymous+missense). duckvep —
the *same* engine plus our patches — is **also 100%** on it (5,044/5,044
shared pairs). 100% concordance on a few thousand SNVs is the easy
regime; it says nothing about the classes where the engine diverges.

**“shared pairs” → the transcript-set difference was a cache-vs-GFF
artifact, now controlled away.** With VEP run on its **cache**, it
emitted ~7,000 fewer `(variant,transcript)` pairs than the GFF-driven
engines (the paper calls these “lncRNA” — in fact they are *mostly
protein-coding*, and VEP also *missed* ~400 the engines emit,
contradicting “no missing transcripts”). Once VEP reads the **same GFF
model** (`--gff`, above), that gap collapses to the handful below, and
the residual is pure engine. Numbers read from
`correctness/data/methodology_audit.csv`:

A shared-pair inner join could *hide* a tool failing to emit a
transcript (it would become a non-shared row, not a discordance), so we
count **every** kind of divergence as first-class: consequence
disagreement on shared pairs, **plus** pairs each tool emits that the
oracle does not (extra), **plus** pairs the oracle emits that the tool
misses. Read from `correctness/data/methodology_audit.csv`:

| divergence type (controlled, 50K ClinVar) | duckvep | fastVEP |
|:------------------------------------------|--------:|--------:|
| consequence disagreement (shared pairs)   |     132 |   5,025 |
| extra pairs the tool emits, VEP does not  |      29 |      31 |
| pairs VEP emits, the tool misses          |       0 |      33 |
| TOTAL divergence vs VEP 116               |     161 |   5,089 |

With the gene model controlled, the emission gap is tiny in both
directions, so the comparison is genuinely pure-engine — and counting
*all* divergence (consequence + extra + missing), **duckvep totals 161
vs fastVEP 5,089** of ~1.43M pairs. The duckvep residual is dominated by
consequence-engine edges (start-codon straddle, boundary insertion); the
27 extras are 5′/3′-distance-window boundary cases. The fastVEP paper’s
100% held only because its validation set is 173 SNVs.

## Synthetic hard-variant corpus

Random ClinVar sampling almost never places an indel *exactly* on a
splice landmark, so it cannot measure the splice/indel fixes.
`tools/gen-hard-variants` (a Rust binary using noodles + the parquet
cache — no R/bash/Python) constructs a corpus of pathological variants
at **exact** offsets computed from the transcript model —
deletions/insertions spanning the donor, acceptor, 5th base, donor
region, polypyrimidine tract and splice region, plus CDS
frameshift/inframe indels and in-codon MNVs — to stress-test those
classes directly.

## Reproduce

``` sh
# 0. fetch all inputs with pinned provenance (Ensembl 116, ClinVar 2026-06-06,
#    VEP cache 116, GIAB) -> data/ ; see scripts/fetch-data.R for exact sources
scripts/fetch-data.R
# 1. build the Ensembl-sourced cache (server-free, from MySQL dumps)
correctness/cache-build/build-cache.R homo_sapiens 116 38
# 2. impact-stratified concordance vs offline Ensembl VEP
correctness/vep_concordance.R data/giab/clinvar.vcf.gz \
  data/giab/GRCh38.116.gff3.gz data/giab/GRCh38.primary.fa 50000
# 3. synthetic hard-variant corpus (Rust + noodles)
cargo build -p duckvep-tools --release
./target/release/gen-hard-variants \
  data/cache/homo_sapiens.116.38/exons.parquet \
  data/cache/homo_sapiens.116.38/transcripts.parquet \
  data/giab/chr17.fa 17 correctness/data/hard_corpus.vcf
# 4. render this report
make correctness
```

See [`../docs/PATCHES.md`](../docs/PATCHES.md) for the accuracy patches
and [`cache-build/README.md`](cache-build/README.md) for the cache
builder.
