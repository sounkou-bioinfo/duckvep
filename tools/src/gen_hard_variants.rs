...
fn cstr<'a>(b: &'a RecordBatch, n: &str) -> &'a StringArray {
    b.column(b.schema().index_of(n).unwrap()).as_any().downcast_ref().unwrap()
}
...
fn main() -> Result<()> {
    ...
    for batch in ParquetRecordBatchReaderBuilder::try_new(File::open(tx_p)?)?.build()? {
        let b = batch?;
        let (tid, ch, bt) = (cstr(&b, "transcript_id"), cstr(&b, "chrom"), cstr(&b, "biotype"));
        let (mane, canon) = (cbool(&b, "mane_select"), cbool(&b, "canonical"));
        for i in 0..b.num_rows() {
            if ch.value(i) == chrom && bt.value(i) == "protein_coding" && (mane.value(i) || canon.value(i)) {
    ...
    for batch in ParquetRecordBatchReaderBuilder::try_new(File::open(exons_p)?)?.build()? {
        let b = batch?;
        let (tid, s, e) = (cstr(&b, "transcript_id"), ci64(&b, "start"), ci64(&b, "end"));
        for i in 0..b.num_rows() {
            let t = tid.value(i);
            if keep.contains(t) {
                exons.entry(t.to_string()).or_default().push((s.value(i), e.value(i)));
            }
        }
    }
    ...
    for ex in exons.values() {
        ...
        for w in ex.windows(2) {
            ...
            del(&fa, &chrom, &mut recs, istart - 1, istart + 1, "del_span_donor5p");
            del(&fa, &chrom, &mut recs, istart + 3, istart + 5, "del_donor_region");
            del(&fa, &chrom, &mut recs, istart + 4, istart + 4, "del_5th_base");
            del(&fa, &chrom, &mut recs, iend - 1, iend + 1, "del_span_acceptor3p");
            del(&fa, &chrom, &mut recs, iend - 12, iend - 8, "del_polypyrimidine");
            del(&fa, &chrom, &mut recs, istart + 2, istart + 7, "del_splice_region");
            ins(&fa, &chrom, &mut recs, istart + 1, "AT", "ins_donor");
            ins(&fa, &chrom, &mut recs, iend - 3, "GC", "ins_acceptor_region");
            del(&fa, &chrom, &mut recs, istart - 1, istart - 1, "del_fs1_exonend");
            del(&fa, &chrom, &mut recs, istart - 4, istart - 2, "del_inframe3_exonend");
            if let Some(r) = seq(&fa, &chrom, iend + 6, iend + 8) {
                if r.len() == 3 {
                    let comp: String = r.chars().map(|c| match c {
                        'A' => 'C', 'C' => 'G', 'G' => 'T', 'T' => 'A', o => o,
                    }).collect();
    ...
    eprintln!("generated {} hard variants across {} transcripts", recs.len(), exons.len());
    Ok(())
}
