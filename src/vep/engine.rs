...
fn annotate_all(
    ...
    while reader.read_record(&mut record)? != 0 {
        ...
        let position = GenomicPosition::new(chrom.clone(), pos, end, Strand::Forward);
        let query_start = pos.saturating_sub(distance).max(1);
        let query_end = end + distance;
        let overlapping = ctx
            .transcripts
        ...
        let result = ctx.predictor.predict(
            &position,
            &ref_allele,
            &alt_alleles,
            &overlapping,
